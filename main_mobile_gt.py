...
def main(running):
    ...
    path = "/Users/allenzhu/Desktop/Research/Video/Wildtrack_dataset/annotations_positions/"

    while running[0]:
        img, frame_id, frame_name, meta = reader.get_data()
        if not len(img):
            break
        ...
        frame_name = re.findall(r'\d+',frame_name.split("/")[-1])[0]
        if frame_name == "00002000":
            continue
        with open(path+ frame_name +".json") as j:
            data = json.load(j)
